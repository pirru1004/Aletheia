...
// Remove the floating panel completely. Regex might be tricky, so let's just use string slicing if needed.
// index.html has:
//       <!-- Floating Predictive Chart Panel -->
//       <div id="floating-chart-panel" class="floating-panel hidden">
//        ...
//       </div> (around line 200)
// Let's replace the whole section safely:
indexHTML = indexHTML.replace(/<!-- Floating Predictive Chart Panel -->[\s\S]*?<\/select>\s*<\/div>\s*<\/div>\s*<\/div>/, '');

// Inject the modal before </body>
const modalHTML = `
  <!-- Full Screen Aletheia Report Modal -->
  <div id="aletheia-report-modal">
